...
open class VersaPlayerControlsBehaviour {
    ...
    public func defaultActivationBlock() {
        ...
        controls.controlsCoordinator.gestureReciever.isHidden = true
        self.controls.handler.context.setNeedsFocusUpdate()
    ...
    public func defaultDeactivationBlock() {
        UIView.animate(withDuration: 0.3, animations: {
            ...
            if $0 {
                ...
                self.controls.controlsCoordinator.gestureReciever.isHidden = false
                self.controls.handler.context.setNeedsFocusUpdate()
            }
        })
    }
